<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="project_Picklist.aspx.cs" Inherits="DDpage.KmrStorage.project_Picklist" %>
...
14         text-align: left;
    ...
    <script>
        ...
        //查询按钮还原
        function resetBtn() {
            if ( _listFourStat == 1 && _listFiveStat==1) {
                mui("#queryBtn").button('reset');
            }
        ...
        function getFiveList() {
            ...
            mui.ajax(ajaxUrl, {
                data: { comName: _company, matName: strMatName, stockName: strStockName, dataNum: strdataNum, facCode: _facCode},
                async: false,
                crossDomain: true,
                type: 'post',//HTTP请求类型
                success: function (data) {
                    ...
                    for (var i = 0; i < js.length; i++) {
                        ...
                        strHtml += "<div class=\"list_info\"><div>库存地点：<span class=\"stockfive" + js[i].matID + "\">" + js[i].matStock + "</div><div>领料类型：<span class=\"typefive" + js[i].matID + "\">工程类领料</div></div>";
                        strHtml += "<div class=\"list_info\"><div>库存：<span class=\"numfive" + js[i].matID + "\">" + js[i].matNum + "</div><div>领料数量:<input type=\"text\" style=\"width: 5em; height: 1em; margin: 0; font - size:0.5em\" class=\"pnumfive" + js[i].matID + "\" value=\"\"></div></div>";
                        strHtml += "</div>";
        ...
        function postData() {
            ...
            $.post("../api/KmrStorage/Post_ProjectPickSubmit_BL.aspx",
                { PCompany: _company, PDept: _dept, PVenture: _venture, SapVenId: _venSapId, PName: _name, PCode: _cardId, PCostCode: _costCode, OddNo: orderNum, ProNo: '',  dataListFour: strFourJson, FourNum: fourNum },
