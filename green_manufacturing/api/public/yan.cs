...
namespace TDM
{
	public class yan
	{
        ...
        public string user_code(string phone) 
        {
            ...
            DBCM.CommandText = "select * from tdm_public_personinfo where 联系电话='"+phone+"' ";
            DBDA.SelectCommand = DBCM;
            DBDA.Fill(lst);
            close_db();
            if (lst.Rows.Count > 0) 
            {
                re = lst.Rows[0]["身份证号"].ToString() + ","+ lst.Rows[0]["公司"].ToString() + "," + lst.Rows[0]["部门"].ToString() + "," + lst.Rows[0]["姓名"].ToString() + "," + lst.Rows[0]["经营体名称"].ToString() + "," + lst.Rows[0]["班组"].ToString();
            }
            return re;
        }
        ...
        public string user_card(string phone)
        {
            ...
            open_db1();
            DataTable lst = new DataTable();
            DBCM.CommandText = "select cardno from tdm_public_personinfo where  联系电话='" + phone + "' ";
            DBDA.SelectCommand = DBCM;
            DBDA.Fill(lst);
            close_db();
            if (lst.Rows.Count > 0)
        ...
        {
            string re = "";
            string access_token = EnterpriseBusiness.GetToken(Config.ECorpId,Config.ECorpSecret);
